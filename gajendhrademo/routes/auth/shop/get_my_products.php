...
try {
    if ($shop_id > 0) {
        $stmt = $conn->prepare("
            SELECT 
                p.id, p.name, p.slug, p.description, p.product_description,
                p.benefits, p.how_to_use, p.variations, p.stock, p.status,
...
while ($row = $result->fetch_assoc()) {
    ...
    $ts_stmt = $conn->prepare(
        "SELECT id, title, description, icon FROM taste_segment WHERE product_id = ?"
    );
