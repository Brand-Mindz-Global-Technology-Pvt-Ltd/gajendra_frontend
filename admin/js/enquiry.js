...
function applyFiltersAndRender() {
    ...
    const typeValue = document.getElementById("enquiryTypeFilter")?.value || "all";
    const dateValue = dateFilter?.value || "all";

    filteredEnquiries = allEnquiries.filter((item) => {
        // Search filter
        const combined =
            `${item.name} ${item.email} ${item.phone} ${item.message || ""} ${item.remarks || ""}`.toLowerCase();
        if (searchTerm && !combined.includes(searchTerm)) return false;

        // Type filter
        if (typeValue !== "all") {
            if (item.enquiry_type !== typeValue) return false;
        }
...
function renderTable() {
    ...
    pageItems.forEach((item, idx) => {
        ...
        const isBulk = item.enquiry_type === "bulk";
        const typeBadge = isBulk
            ? '<span class="badge bg-warning text-dark"><i class="fas fa-box-open mr-1"></i> Bulk</span>'
            : '<span class="badge bg-info"><i class="fas fa-user mr-1"></i> Contact</span>';

        const previewText = isBulk ? (item.remarks || "Bulk Enquiry Request") : (item.message || "-");
      ...
      <tr id="enqRow_${item.id}">
        ...
        >
          ${escapeHtml(truncateMessage(previewText, 80))}
        </td>
        <td>${formattedDate}</td>
    ...
    if (paginationWrapper && pageInfo) {
        ...
        pageInfo.textContent = `Showing ${startIndex + 1
            }–${Math.min(endIndex, filteredEnquiries.length)} of ${filteredEnquiries.length
            } enquiries`;
    }
}
...
function openMessageModal(id) {
    ...
    const isBulk = enquiry.enquiry_type === "bulk";

    if (modalType) {
        modalType.textContent = isBulk ? "BULK ENQUIRY" : "CONTACT ENQUIRY";
        modalType.className = isBulk ? "badge bg-warning text-dark" : "badge bg-info";
    }
    ...
    if (isBulk) {
        ...
        if (deliveryDateEl) deliveryDateEl.textContent = enquiry.delivery_date || "Not specified";
        if (categoryEl) categoryEl.textContent = (enquiry.category_id && enquiry.category_id !== "0") ? enquiry.category_id : "-";
        if (productEl) productEl.textContent = (enquiry.product_id && enquiry.product_id !== "0") ? enquiry.product_id : "-";
        if (addressEl) addressEl.textContent = enquiry.address || "Not specified";
        if (remarksEl) remarksEl.textContent = enquiry.remarks || "No remarks provided";
    } else {
