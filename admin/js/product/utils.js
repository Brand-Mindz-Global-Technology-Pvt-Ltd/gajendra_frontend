...
function initializeImageUploadSlots() {
  ...
  if (container && container.innerHTML.trim() === '<!-- Image upload slots will be generated here -->') {
  ...
  document.querySelectorAll('.image-upload-slot').forEach((slot) => {
    ...
    if (undoBtn) {
        undoBtn.addEventListener('click', function (e) {
          e.stopPropagation();
          slot.classList.remove('deleted-slot');
          // Logic handled by caller or just state update
          if (preview.src && preview.src !== '') {
             // If we have a preview (e.g. from edit mode), show it
             preview.style.display = 'block';
             if (placeholder) placeholder.style.display = 'none';
             slot.classList.add('has-image');
             if (removeBtn) removeBtn.style.display = 'block';
  ...
  console.log('✅ Image upload slots initialized');
}
